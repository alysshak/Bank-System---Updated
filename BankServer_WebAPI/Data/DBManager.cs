        ...
        public static bool InsertUserProfile(UserProfile userProfile)
        {
            try
            {
                using (SQLiteConnection connection = new SQLiteConnection(connectionString))
                {
                    ...
                    {
                        command.CommandText = @"
                        INSERT INTO UserProfiles (FirstName, LastName, Username, Email, Address, Phone, Picture, Password)
                        VALUES (@FirstName, @LastName, @Username, @Email, @Address, @Phone, @Picture, @Password)";
